<template>
    <div>
        ...
        <el-input class = "search-input" type = "text" v-model = "searchString" :placeholder="$t('home.homeHeadSearchInputPlaceholderText')">
            <template #prefix>
                <el-icon class = "el-input__icon" :size = "20" ><search /></el-icon>
            </template>
        </el-input>
        <el-button class = "search-button" @click="clickOnSearch">{{$t('home.homeHeadSearchButtonText')}}</el-button>
        <el-dropdown class = "avatar-container">
        <el-avatar class = "user-avatar" :src="userAvatar"></el-avatar>
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item @click = "clickOnPersonalInformation">{{$t('home.homeHeadPersonalInformationItemText')}}</el-dropdown-item>
            <el-dropdown-item divided v-if="showAppManagement" @click = "clickOnAppManagement">{{$t('home.homeHeadAppManagementItemText')}}</el-dropdown-item>
            <el-dropdown-item divided v-if="showUserManagement" @click = "clickOnUserManagement">{{$t('home.homeHeadUserManagementItemText')}}</el-dropdown-item>
            <el-dropdown-item divided @click = "clickOnLogOut">{{$t('home.homeHeadLogOutItemText')}}</el-dropdown-item>
          </el-dropdown-menu>
        </template>
      </el-dropdown>
    </div>
        ...
        mounted() {
            ...
            NetApiShareInstance.userInformation(readAccount(), readToken()).then((res) => {
                if (res.data.ret === 0) {
                    ...
                    errorNotificationShow(this.$t('home.homeHeadObtainUserInformationErrorNotificationText'), res.data.message)
                }
            }).catch((err) => {
